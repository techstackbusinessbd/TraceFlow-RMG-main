...
| `v2.4` | 2026-09-07 | Product Owner & RMG Architect | **এমপ্লয়ী আইডি (`emp_id`) পলিসি আপডেট:** সিস্টেমের সমস্ত বিজনেস এনটিটি কোড অটো-জেনারেটেড হলেও **শুধুমাত্র এমপ্লয়ী আইডি (`emp_id`) কোনো অটো-কোড হবে না; এটি ফ্যাক্টরি এইচআর/আইডি কার্ড অনুযায়ী সরাসরি ম্যানুয়াল ইনপুট হবে** (যেমন: `255776`, `10492`)। |
...
- **REQ-AUTH-001: ট্রিপল-আইডেন্টিফায়ার প্রমাণীকরণ ও ম্যানুয়াল এমপ্লয়ী আইডি নীতি (Tri-Identifier Auth & Manual Emp ID Policy):**
  - **এমপ্লয়ী আইডি ম্যানুয়াল ইনপুট স্ট্যান্ডার্ড (STRICT EXCEPTION TO AUTO-CODE):**
    - পুরো ইআরপি সিস্টেমে বায়ার, স্টাইল, পিও বা রোল কোডগুলো সিস্টেম দ্বারা অটো-জেনারেটেড হলেও **একমাত্র ইউজার এমপ্লয়ী আইডি (`emp_id`) কোনো সিস্টেম অটো-জেনারেটেড কোড হবে না**।
    - প্রতিটি কারখানায় কর্মীদের নিজস্ব বায়োমেট্রিক পাঞ্চ কার্ড, এইচআর ডেটাবেস এবং গলায় ঝুলানো আইডি কার্ডে পূর্বনির্ধারিত অফিসিয়াল আইডি থাকে (যেমন: `255776`, `10492`, `88301`)।
    - ইউজার তৈরি বা রেজিস্ট্রেশনের সময় অ্যাডমিন সরাসরি ফ্যাক্টরি আইডি কার্ড দেখে ম্যানুয়ালি `emp_id` ইনপুট প্রদান করবেন (`editable manual text field`)।
    - ব্যাকএন্ড সার্ভার সাইডে ভ্যালিডেশন নিশ্চিত করবে যে সক্রিয় অ্যাকাউন্টগুলোর মধ্যে `emp_id` যেন ১০০% ইউনিক হয় (`unique:users,emp_id`).
  - **ট্রিপল-আইডেন্টিফায়ার লগইন ইঞ্জিন:**
    ...
    - সিস্টেম স্বয়ংক্রিয়ভাবে ইনপুট রেজলভ করবে:
      ...
      2. অন্যথায় এটি সমান্তরালভাবে `users.emp_id` (e.g. `255776`) এবং `users.username` ফিল্ডে কেস-ইনসেনসিটিভ লুকআপ করবে।
    - যেসকল ফ্যাক্টরি কর্মীর ইমেইল নেই, তারা অনায়াসে তাদের পরিচিত ম্যানুয়াল `emp_id` (e.g. `255776`) অথবা `username` (e.g. `cutting_master_01`) দিয়ে সিস্টেমে প্রবেশ করতে পারবেন।
